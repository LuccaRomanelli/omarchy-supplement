...
# Centralized curl-based package updater
# Usage: ./update-package.sh <package_name>
# Re-runs the installer (most curl-based installers support auto-update)
...
# Parse the entry (name, check_command, install_url)
CHECK_CMD=$(echo "$PACKAGE_ENTRY" | awk '{print $2}')
INSTALL_URL=$(echo "$PACKAGE_ENTRY" | awk '{print $3}')

if [ -z "$CHECK_CMD" ] || [ -z "$INSTALL_URL" ]; then
    echo "Error: Invalid package entry for '$PACKAGE_NAME'"
    echo "Expected format: <name> <check_command> <install_url>"
    exit 1
fi
